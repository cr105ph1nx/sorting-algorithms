...
int *quickSortInt(int *array, int low, int high, int option){
  int partition (int *array, int low, int high, int option){
    if(option == 0){
  ...
  if (low < high){
    ...
    quickSortInt(array, low, pi - 1, option);
    quickSortInt(array, pi + 1, high, option);
  }
  return(array);
}
